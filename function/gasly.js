...
function handleDistances(distanceWithStopover, distanceWithoutStopover) {
  ...
  console.log(((distanceWithStopover - distanceWithoutStopover) / (typical_mileage * 1000) + typical_fill ) * gas_prices.get("detour"));
  console.log(typical_fill * gas_prices.get("path"));
  if (gas_prices.get("path") < gas_prices.get("detour")){
    console.log('Gas station on path has cheaper gas at:', gas_prices.get("path"));
    return false
  } else if ((((distanceWithStopover - distanceWithoutStopover) / (typical_mileage * 1000) + typical_fill ) * gas_prices.get("detour")) < (typical_fill * gas_prices.get("path"))){
